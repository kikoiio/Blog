...
- **左侧边栏**：返回首页箭头 + 目录（TOC，min-width 240px）
- **主内容区**：文章正文（article-header 已隐藏，避免空白色条）
- **右侧边栏**：像素猫群 canvas（6-8只不同品种的像素猫）
...
右侧边栏的 canvas 上有 6-8 只随机分布的像素猫，每只有不同的品种配色（橘猫、黑猫、白猫、灰猫、三花猫、暹罗猫、燕尾服猫、姜黄猫）。

**精灵图**：使用 16x14 字符网格定义，每个字符映射到颜色（O=轮廓, B=基色, S=阴影, E=耳内, Y=眼睛, N=鼻子）。`PX=3` 即每个精灵像素渲染为 3x3 CSS 像素。

**姿态**：
- `SPRITE_SIT` — 静坐（默认 idle）
- `SPRITE_HAPPY` — 开心（眯眼微笑）
- `SPRITE_SCARED` — 惊吓（圆眼张嘴）
- `SPRITE_WALK1/2` — 行走（两帧交替）
- `SPRITE_RUN1/2` — 奔跑（两帧交替，身体前倾）

**点击交互**：
1. 计算距离点击位置最近的 2-3 只猫
2. 每只猫随机分配反应：
   - **平静 (35%)**：缓慢走向点击位置，到达后显示静坐姿态
   - **开心 (35%)**：走向点击位置，到达后显示开心表情
   - **惊吓 (30%)**：先显示惊吓表情 0.6 秒，然后朝远离点击的方向奔跑
3. 猫之间有碰撞检测，避免重叠
