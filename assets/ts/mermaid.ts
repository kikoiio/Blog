...
function buildBaseConfig(cfg: MermaidConfig): Record<string, any> {
    const base: Record<string, any> = {
        ...
        // Keep native diagram width and let the wrapper handle scrolling.
        flowchart: { htmlLabels: cfg.htmlLabels ?? true, useMaxWidth: false },
        gantt: { useWidth: 800 },
    };
...
function getSvgNativeSize(svg: SVGElement) {
...
function applyReadableSizing(el: HTMLElement) {
    const svg = el.querySelector('svg');
    if (!svg) return;

    const { width } = getSvgNativeSize(svg);
    const hostWidth = el.clientWidth || el.getBoundingClientRect().width || width;
    const isWide = width > hostWidth * WIDE_DIAGRAM_RATIO;
...
function applyReadableSizingToAll(elements: NodeListOf<HTMLElement>) {
    elements.forEach((el) => applyReadableSizing(el));
}
...
export async function initMermaidPage(config: MermaidConfig) {
    ...
    applyReadableSizingToAll(elements);
    ...
    window.addEventListener('onColorSchemeChange', async () => {
        ...
        elements.forEach((el, i) => {
            ...
            applyTransparency(el, i);
            applyReadableSizing(el);
        });
    });

    window.addEventListener('resize', () => applyReadableSizingToAll(elements));
}
