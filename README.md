...
导航行为规则（由 `layouts/home.html` 与 `assets/ts/graph/tree.ts` 决定）：

- 目录节点在力导向图中逐层展开，层级可以一直向下展示；节点上的数字徽标显示其下笔记总数。
- 当一个目录**直接**包含 2 篇及以上笔记时，点击它改为打开按名称稳定排序的笔记索引（索引递归收录所有后代笔记）；直接笔记不足 2 篇的目录始终以节点形式展开。
- 带附件的笔记（页面包：同名文件夹 + `index.md` + 配图）算作其所在目录的一篇直接笔记，不会在导航图中多出节点。因此摄影相册以 `Photography` 索引的形式呈现。
...
3. 有配图或其他附件时，把 `笔记.md` 改成同名文件夹 + `index.md`（页面包），附件放同目录，正文用相对路径引用（Obsidian 写法直接兼容）。页面包仍然是一篇文章：它计入所在目录的直接笔记数，不会单独出现在导航图里。
